...
import numpy as np
import pandas as pd
# Ensure matplotlib cache is writable in notebook/process-worker contexts.
_mpl_cfg = os.environ.setdefault('MPLCONFIGDIR', '/tmp/matplotlib')
try:
...
def pressure_calc(filename=None, wb=None, residual_threshold=5.0, embed_plot=True):
    """
    Estimate pressure from workbook data using residual curves and parabolic fitting.

    This implementation auto-detects phase sheets so it can handle:
      - quartz + feldspar (2-phase only), and
      - quartz + feldspar + feldspar_1/feldspar_2/... (3-phase available).

    Parameters:
        filename (str): Excel file path (optional if wb is provided).
        residual_threshold (float): Max acceptable residual to accept pressure estimate.
        embed_plot (bool): Whether to generate and embed residual plots.
    ...
    if 'init_cond' not in sheet_names:
        raise ValueError("Workbook missing required sheet 'init_cond'")

    # Gather phase tabs from workbook instead of hard-coding feldspar_1.
    phase_sheets = [
        s for s in sheet_names
        if s.lower() == 'quartz' or s.lower().startswith('feldspar')
    ]
    if 'quartz' not in [s.lower() for s in phase_sheets]:
        raise ValueError("Workbook missing required phase sheet 'quartz'")
    if not any(s.lower().startswith('feldspar') for s in phase_sheets):
        raise ValueError("Workbook missing required feldspar phase sheet")
    ...
    phase_sheets = sorted(phase_sheets, key=_phase_sort_key)

    dfs = {'init_cond': pd.read_excel(xls, sheet_name='init_cond', header=None)}
    for phase in phase_sheets:
        dfs[phase] = pd.read_excel(xls, sheet_name=phase, header=0)
    ...
    # Build results DataFrame: T vs Pressure for each detected phase sheet
    results = pd.DataFrame(index=pressures)
    for phase in phase_sheets:
        df = dfs[phase]
        temps = []
        for p in pressures:
            if 'P (MPa)' not in df.columns or 'T (C)' not in df.columns:
                temps.append(np.nan)
                continue
            match = df[df['P (MPa)'] == p]
            if not match.empty:
    ...
    delta_2 = np.array(delta_2, dtype=float)
    delta_3 = np.array(delta_3, dtype=float)

    def fit_and_plot(P, residuals, title, subplot_ax=None):
        valid = residuals[~np.isnan(residuals)]
        if len(valid) == 0:
            print(f"No valid residual points for {title.lower()}")
        ...
        try:
            ...
            if np.isclose(a, 0.0, atol=1e-10):
                print(f"Parabola fit is effectively linear for {title.lower()} (a≈0); treating as fit failure")
    ...
    if embed_plot:
        ...
        # Top left: 2-phase TP grid (quartz + first feldspar-like)
        ...
        # Top right: 3-phase TP grid (if available)
        ...
        if results.shape[1] >= 3:
            ...
            axs[0, 1].text(0.5, 0.5, 'No third phase sheet available', ha='center', va='center')
    ...
    # Add a worksheet for pressure analysis results
    if wb is None:
        raise ValueError("pressure_calc requires an openpyxl workbook via `wb` for writing results")
        ...
        for step in range(N_runs + 1):
            ...
            try:
                ...
                step_data = {
                    ...
                    'calc_time': t1 - t0,
                    'system_properties': {
                        'Mass': state.properties(phase_name="System", props="Mass"),
                        'Density': state.properties(phase_name="System", props="Density"),
                        'GibbsFreeEnergy': state.properties(phase_name="System", props="GibbsFreeEnergy"),
                        'Enthalpy': state.properties(phase_name="System", props="Enthalpy"),
                        'Entropy': state.properties(phase_name="System", props="Entropy"),
                        'HeatCapacity': state.properties(phase_name="System", props="HeatCapacity"),
                        'Volume': state.properties(phase_name="System", props="Volume"),
                        'DvDt': state.properties(phase_name="System", props="DvDt"),
                        'DvDp': state.properties(phase_name="System", props="DvDp"),
                        'D2vDt2': state.properties(phase_name="System", props="D2vDt2"),
                        'D2vDp2': state.properties(phase_name="System", props="D2vDp2"),
                        'D2vDtDp': state.properties(phase_name="System", props="D2vDtDp"),
                    },
                    'liquid_properties': {
                        'Mass': state.properties(phase_name="Liquid", props="Mass"),
                        'Density': state.properties(phase_name="Liquid", props="Density"),
                        'GibbsFreeEnergy': state.properties(phase_name="Liquid", props="GibbsFreeEnergy"),
                        'Enthalpy': state.properties(phase_name="Liquid", props="Enthalpy"),
                        'Entropy': state.properties(phase_name="Liquid", props="Entropy"),
                        'HeatCapacity': state.properties(phase_name="Liquid", props="HeatCapacity"),
                        'Volume': state.properties(phase_name="Liquid", props="Volume"),
                        'DvDt': state.properties(phase_name="Liquid", props="DvDt"),
                        'DvDp': state.properties(phase_name="Liquid", props="DvDp"),
                        'D2vDt2': state.properties(phase_name="Liquid", props="D2vDt2"),
                        'D2vDp2': state.properties(phase_name="Liquid", props="D2vDp2"),
                        'D2vDtDp': state.properties(phase_name="Liquid", props="D2vDtDp"),
                    },
                    'liquid_composition': state.oxide_comp('Liquid'),
                ...
                for solid_name in phases_present:
                    ...
                    try:
                        solid_data = {
                            'Mass': state.properties(phase_name=solid_name, props="Mass"),
                            'Density': state.properties(phase_name=solid_name, props="Density"),
                            'GibbsFreeEnergy': state.properties(phase_name=solid_name, props="GibbsFreeEnergy"),
                            'Enthalpy': state.properties(phase_name=solid_name, props="Enthalpy"),
                            'Entropy': state.properties(phase_name=solid_name, props="Entropy"),
                            'HeatCapacity': state.properties(phase_name=solid_name, props="HeatCapacity"),
                            'Volume': state.properties(phase_name=solid_name, props="Volume"),
                            'DvDt': state.properties(phase_name=solid_name, props="DvDt"),
                            'DvDp': state.properties(phase_name=solid_name, props="DvDp"),
                            'D2vDt2': state.properties(phase_name=solid_name, props="D2vDt2"),
                            'D2vDp2': state.properties(phase_name=solid_name, props="D2vDp2"),
                            'D2vDtDp': state.properties(phase_name=solid_name, props="D2vDtDp"),
                            'composition': state.oxide_comp(solid_name)
                        }
...
def process_single_composition_parallel(comp_data):
    ...
    try:
        ...
        def _run_tasks_serial(task_list):
            serial_results = []
            for task in task_list:
                try:
                    result = run_single_pressure_step(task)
                    serial_results.append(result)
                    if result.get('success', False):
                        if verbose:
                            print(f"  ✓ P={result['pressure']} MPa: {result['num_steps']} temperatures calculated (serial)")
                    else:
                        if verbose:
                            print(f"  ✗ P={result.get('pressure', 'unknown')} MPa: Failed - {result.get('error', 'Unknown error')} (serial)")
                except Exception as exc:
                    if verbose:
                        print(f"Pressure step failed for {label} (serial): {exc}")
            return serial_results

        # Run pressure steps in parallel when available; fall back to serial on process-spawn failures.
        max_workers = min(max_pressure_workers, len(tasks))
        if max_workers <= 1:
            results = _run_tasks_serial(tasks)
        else:
            try:
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    futures = [executor.submit(run_single_pressure_step, task) for task in tasks]
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if verbose:
                                print(f"Pressure step failed for {label}: {exc}")
            except (OSError, PermissionError) as exc:
                if verbose:
                    print(f"Process pool unavailable ({exc}); falling back to serial pressure-step execution.")
                results = _run_tasks_serial(tasks)
        ...
        import os
        os.makedirs('parallel-results', exist_ok=True)
        
        P2 = (None, None, None)
        P3 = (None, None, None)
        pressure_status = "skipped"
        if run_pressure_analysis:
            wb, P2, P3 = pressure_calc(wb=wb)
            pressure_status = "computed"
        filename = f'parallel-results/Parallel_MELTS_{label}_dP{params["delta_P"]}_{month_day}_{max_pressure_workers}cores.xlsx'
        wb.save(filename)
        ...
        if verbose:
            ...
            if run_pressure_analysis:
                ...
                print("Pressure analysis skipped (run_pressure_analysis=False)")
            if all_solid_phases:
...
def parallel_melts_main_loop(
    ...
    """
    Main function to run MELTS calculations with parallel processing.
    This function is in the module so it can be pickled properly.
    ...
    - run_pressure_analysis: If False, skip pressure_calc and write thermodynamic sheets only
    """
    import pandas as pd
    import numpy as np
    from concurrent.futures import ProcessPoolExecutor, as_completed
    ...
    if verbose:
        ...
        if not run_pressure_analysis:
            print("Data-only mode enabled: pressure analysis will be skipped.")
    t0 = time.time()
    
    # Load compositions
    try:
        comps = pd.read_csv(compositions_csv, index_col=0).replace(np.nan, 0, regex=True)
        ...
        comps = pd.read_csv(compositions_csv, index_col=0, on_bad_lines='skip').replace(np.nan, 0, regex=True)
    labels = list(comps)
    if verbose:
    ...
    for label in labels:
        curCol = comps[label]
        composition = pd.to_numeric(curCol[:15]).to_dict()
        
        # Skip if missing required components
        if composition['SiO2'] == 0 or composition['Al2O3'] == 0:
            if verbose:
        ...
        params = {
            'T1': float(curCol['T1']),
            'T2': float(curCol['T2']),
            'delta_T': float(curCol['ΔT']),
            'P1': float(curCol['P1']),
            'P2': float(curCol['P2']),
            'delta_P': float(curCol['ΔP']),
            'const_fO2': curCol['fO2 constraint'],
            'fO2Path': curCol['fO2 buffer'],
            'fO2_offset': float(curCol['fO2 offset'])
        }
        ...
        try:
            ...
            composition_tasks.append(
                (label, composition, params, blk_cmp, max_pressure_workers, verbose, run_pressure_analysis)
            )
    ...
    if max_composition_workers > 1:
        if verbose:
            print(f"Processing {max_composition_workers} compositions in parallel...")
        with ProcessPoolExecutor(max_workers=max_composition_workers) as executor:
            futures = [executor.submit(process_single_composition_parallel, task) for task in composition_tasks]
...
def test_pressure():
    ...
    def muH2O(t, p, state):
        return Water.gibbs_energy(t, p)
    comps = pd.read_csv(CSV_Filename,index_col=0).replace(np.nan, 0, regex=True)
    labels = list(comps)
